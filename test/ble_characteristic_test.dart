...
BleService mockBleService = BleService(
  uuid: serviceId,
  characteristics: [mockBleCharacteristic],
);

void main() {
  late PrinterConnectPlatform platform;
  setUp(() {
    platform = _PrinterConnectMock();
    PrinterConnect.setInstance(platform);
  });

  group('BleCharacteristic Tests', () {
    test("DiscoverServices test", () async {
      debugPrint("Discovering services");
      List<BleService> services = await PrinterConnect.discoverServices(
        mockDeviceId,
      );
...
class _PrinterConnectMock extends PrinterConnectPlatformMock {
  Timer? notifierTimer;
  Uint8List? charValue;

...
默认情况下，所有命令都在全局队列（`QueueType.global`）中执行，每个命令等待上一个命令完成。虽然这种方法较慢，但它是避免命令异常的最安全方法，因此是默认设置。

如果你想在多个设备之间并行化命令，可以设置：
...
你可以通过传递可选的 `queueId` 来拥有单独的队列。具有相同 `queueId` 的命令会被一起序列化，但与 `QueueType.perDevice` 和 `QueueType.global` 并行运行：
...
你也可以通过使用以下方法完全禁用队列并批量执行所有命令，即使对于同一设备也是如此：
...
请记住，某些平台（例如 Android）可能无法很好地处理无法在最小间隔内处理连续命令的设备。因此，不建议将 `queueType` 设置为 `none`。
...
// 清除每个设备的队列（当 queueType 为 perDevice 时）
PrinterConnect.clearQueue(deviceId);

// 清除自定义队列（与传递给 read/write/etc. 的 queueId 相同的字符串）
...
你也可以在发送命令时指定 `timeout` 参数。这将覆盖全局超时。
